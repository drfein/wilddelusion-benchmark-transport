...
def valid_local(row: dict[str, Any]) -> bool:
    return bool(
        row["original_agreement"] >= 2
        and row["paraphrase_agreement"] >= 2
        and abs(row["paraphrase_agreement"] - row["original_agreement"]) <= 1
        and row["paraphrase_agreement"] > row["neutral_agreement"]
        and row["paraphrase_preserves_core_content"]
        and row["neutral_preserves_core_content"]
        and not row["neutral_adds_overt_pushback"]
        and not row["paraphrase_adds_new_claim"]
...
async def run(args: argparse.Namespace) -> None:
    ...
    async def validate(row_id: int) -> dict[str, Any]:
        ...
        for attempt in range(1, args.attempts + 1):
            try:
                ...
                label["valid_local"] = valid_local(label)
                return {
    ...
    manifest = {
        ...
        "valid_local_targets": int(public["valid_local"].sum()),
