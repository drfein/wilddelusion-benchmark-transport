...
def build_features(
    ...
    release["conversation_hash"] = release.apply(
        lambda row: hashlib.sha256(
            f"{row.get('source', '')}\x1f{row['conversation_id']}".encode()
        ).hexdigest(),
        axis=1,
    )
    ...
    # Self-disclosure is defined only for user content in the frozen rubric.
    chunks.loc[chunks["role"].eq("assistant"), "self_disclosure"] = 0

    for feature in ("delusion_content", "rapport", "self_disclosure"):
        chunks[f"{feature}_weighted"] = (
            chunks[feature].astype(float) * chunks["word_count"]
        )
    ...
    membership["user_delusion_weighted"] = (
        membership["delusion_weighted"] * membership["is_user"]
    )
    ...
    count_columns = [
        "prefix_words",
        "prefix_messages",
        "prefix_assistant_messages",
        "prefix_user_messages",
        "delusion_weighted",
        "rapport_weighted",
        "self_disclosure_weighted",
        "user_words",
        "user_delusion_weighted",
        "delusion_max",
    ]
    features[count_columns] = features[count_columns].fillna(0)
    denominator = 4 * features["prefix_words"].replace(0, np.nan)
    features["delusion_density"] = (features["delusion_weighted"] / denominator).fillna(
        0
    )
    features["rapport_density"] = (features["rapport_weighted"] / denominator).fillna(0)
    features["self_disclosure_density"] = (
        features["self_disclosure_weighted"] / denominator
    ).fillna(0)
    user_denominator = 4 * features["user_words"].replace(0, np.nan)
    features["user_delusion_density"] = (
        features["user_delusion_weighted"] / user_denominator
    ).fillna(0)
    ...
    features["history_type"] = np.select(
        [
            features["prefix_messages"].eq(0),
            features["prefix_assistant_messages"].eq(0),
        ],
...
def clustered_effect(frame: pd.DataFrame, draws: int, seed: int) -> dict[str, Any]:
    ...
    full_low, full_high = proportion_confint(
        int(frame["full_context_endorse"].sum()), len(frame), method="wilson"
    )
    target_low, target_high = proportion_confint(
        int(frame["target_only_endorse"].sum()), len(frame), method="wilson"
    )
    return {
        ...
        "target_only_wilson_low": float(target_low),
        "target_only_wilson_high": float(target_high),
        "full_context_rate": float(frame["full_context_endorse"].mean()),
        "full_context_wilson_low": float(full_low),
        "full_context_wilson_high": float(full_high),
        "pairs": len(frame),
...
def quartile_effects(frame: pd.DataFrame, draws: int, seed: int) -> pd.DataFrame:
    ...
    for feature_index, feature in enumerate(FEATURES):
        try:
            targets["bin"] = pd.qcut(
                targets[feature], 4, labels=False, duplicates="drop"
            )
        except ValueError:
            continue
        ...
        for bin_index, group in expanded.groupby("bin"):
            result = clustered_effect(
                group, draws, seed + 100 * feature_index + int(bin_index)
            )
...
def audit_agreement(
    primary_path: Path | None, audit_path: Path | None
) -> dict[str, Any] | None:
    ...
    joined = primary.merge(
        audit, on="item_id", suffixes=("_primary", "_audit"), validate="one_to_one"
    )
    ...
    expected = float(
        left.mean() * right.mean() + (1 - left.mean()) * (1 - right.mean())
    )
...
def plot(regressions: pd.DataFrame, strata: pd.DataFrame, output: Path) -> None:
    colors = {
        "log_prefix_words": "#2F6690",
        "delusion_density": "#C8553D",
        "rapport_density": "#D99A2B",
        "self_disclosure_density": "#4C956C",
    }
    ...
    for row_index, (_, row) in enumerate(primary.iterrows()):
        color = colors[row["feature"]]
        axes[0].errorbar(
            row["coefficient"] * 100,
            row_index,
            xerr=np.array(
                [
                    [(row["coefficient"] - row["ci_low"]) * 100],
                    [(row["ci_high"] - row["coefficient"]) * 100],
                ]
            ),
            fmt="o",
            color=color,
    ...
    order = [
        "Zero delusion content",
        "Near-zero (<=5%)",
        "Positive delusion content",
        "All nonempty prefixes",
    ]
    panel = strata.set_index("stratum").reindex(order).dropna(subset=["difference"])
    panel = panel.copy()
    degenerate = panel["ci_low"].eq(panel["ci_high"])
    panel.loc[degenerate, "ci_low"] = panel.loc[degenerate, "full_context_wilson_low"]
    panel.loc[degenerate, "ci_high"] = panel.loc[degenerate, "full_context_wilson_high"]
    ...
    axes[1].errorbar(
        ...
        xerr=np.vstack(
            (
                (panel["difference"] - panel["ci_low"]) * 100,
                (panel["ci_high"] - panel["difference"]) * 100,
            )
        ),
        fmt="o",
        color="#315C63",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Analyze heterogeneity in the matched context effect."
    )
    ...
    pairs = pd.read_csv(args.paired_scores)
    pairs = pairs.drop(columns="conversation_hash", errors="ignore").merge(
        features, on="original_row_idx", validate="many_to_one"
    )
    ...
    strata_specs = {
        ...
        "Zero delusion content": pairs["prefix_messages"].gt(0)
        & pairs["zero_delusion"],
        "Near-zero (<=5%)": pairs["prefix_messages"].gt(0)
        & pairs["near_zero_delusion"],
        "Positive delusion content": pairs["prefix_messages"].gt(0)
        & ~pairs["zero_delusion"],
        "User-only prefix": pairs["history_type"].eq("User-only prefix"),
        "Includes assistant history": pairs["history_type"].eq(
            "Includes assistant history"
        ),
    }
    strata = pd.DataFrame(
        [
            {
                "stratum": name,
                **clustered_effect(
                    pairs[mask], args.bootstrap_draws, args.seed + index
                ),
            }
            for index, (name, mask) in enumerate(strata_specs.items())
        ]
    )
        ...
        [
            {
                "model": model,
                **clustered_effect(
                    group, args.bootstrap_draws, args.seed + 2000 + index
                ),
            }
            for index, (model, group) in enumerate(primary.groupby("model"))
        ]
    )
    feature_correlations = features.loc[features["prefix_messages"] > 0, FEATURES].corr(
        method="spearman"
    )
    ...
    safe_features.to_parquet(
        args.out_dir / "prefix_features_no_text.parquet", index=False
    )
    ...
    model_effects.to_csv(
        args.out_dir / "context_effect_by_model_nonempty.csv", index=False
    )
