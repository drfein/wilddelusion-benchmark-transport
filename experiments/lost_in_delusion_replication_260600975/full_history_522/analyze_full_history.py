#!/usr/bin/env python3
"""Cluster-aware analysis of jointly grounded conversational histories."""
...
EXPECTED_JUDGE_PROTOCOL = JUDGE_PROTOCOL
...
def plot_results(summary: pd.DataFrame, output_dir: Path) -> None:
    ...
    available = set(summary["model"])
    models = [
        model
        for model in MODEL_LABELS
        if model in available and model in PAPER_EFFECTS
    ]
    ...
    all_models = [model for model in MODEL_LABELS if model in available]
    figure, axes = plt.subplots(1, 3, figsize=(12.2, 3.4), facecolor="white")
    x = np.arange(len(all_models))
    for axis, metric, title in zip(axes, metrics, titles, strict=True):
        selected = summary[summary["metric"] == metric].set_index("model")
        real = np.array(
            [selected.loc[model, "paired_difference"] for model in all_models]
        )
        low = np.array(
            [
                selected.loc[model, "cluster_bootstrap_ci95_low"]
                for model in all_models
            ]
        )
        high = np.array(
            [
                selected.loc[model, "cluster_bootstrap_ci95_high"]
                for model in all_models
            ]
        )
        axis.bar(
        ...
        axis.set_xticklabels(
            [MODEL_LABELS[model] for model in all_models],
            rotation=28,
            ha="right",
        )
        axis.grid(axis="y", alpha=0.15)
        ...
        axis.spines[["top", "right"]].set_visible(False)
    figure.tight_layout()
    figure.savefig(output_dir / "grounded_real_all_models_clustered.png", dpi=300)
    figure.savefig(output_dir / "grounded_real_all_models_clustered.pdf")
    plt.close(figure)
...
def main() -> None:
    ...
    frame["minimum_pair_preservation"] = frame[preservation_columns].apply(
        pd.to_numeric, errors="coerce"
    ).min(axis=1)
    sensitivity_rows: list[dict[str, Any]] = []
    for threshold in (4, 5):
        threshold_frame = frame[
            frame["minimum_pair_preservation"] >= threshold
        ]
        for model, model_frame in threshold_frame.groupby("model", sort=False):
            for metric in ("DCS ordinal", "HES ordinal", "SIS"):
                ...
                stats["model"] = model
                stats["minimum_pair_preservation"] = threshold
                sensitivity_rows.append(stats)
    pd.DataFrame(sensitivity_rows).to_csv(
        args.output_dir / "preservation_threshold_sensitivity.csv", index=False
    )
    ...
    for model, model_frame in broad_frame.groupby("model", sort=False):
        for metric in ("HES ordinal", "SIS"):
            ...
            stats["model"] = model
            stats["eligibility"] = (
                "original and control harm both in {possible, clear}"
            )
    ...
    write_manifest(
        args.output_dir / "summary.json",
        {
            ...
            "design": (
                "paired real bounded conversation versus a jointly grounded rewrite "
                "of every retained turn; source-conversation-clustered inference"
            ),
